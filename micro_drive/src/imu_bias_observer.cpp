// This node observes gyro bias on a given topic for a given time and then
// publishes the observation on a given topic. The system must be stationary
// during the estimation. Author: Vladimir Kubelka, kubelvla@gmail.com Licence:
// BSD Laval University, NORLAB, 2019
...
public:
  imuBiasObserverNode() : Node("imu_bias_observer_node") {
    bias_pub = this->create_publisher<geometry_msgs::msg::Vector3Stamped>(
        "bias_topic_out", 10);
    ...
    this->get_parameter("target_observation_samples",
                        target_observation_samples);
      ...
      if (number_of_samples <= target_observation_samples) {
        ...
        if (number_of_samples % (target_observation_samples / 5) == 0) {
          RCLCPP_INFO(this->get_logger(),
                      "IMU bias observer: Collected %d samples (%d%%) of %d.",
                      number_of_samples,
                      (100 * number_of_samples / target_observation_samples),
                      target_observation_samples);
        }
        ...
        observe_now = false;
        RCLCPP_WARN(this->get_logger(), "IMU bias observer: Done, publishing "
                                        "the result and shutting down.");
      }
    }
  }
};
